...
const TrackOrderList = () => {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axiosInstance.get(`/getUserOrder?page=${page - 1}&limit=10`);
              ...
              <div class="bg-white shadow">
                <div class="px-4 py-6 sm:px-8 sm:py-10">
                    ...
                    <ul class="-my-8">
                      {order.items.map((scrapInfo, index) => (
                        <li key={index} class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                          <div class="shrink-0">
                            <img class="h-24 w-24 max-w-full rounded-lg object-cover" src={scrapInfo?.scrapInfo.docUrl} alt="" />
                          </div>

                          <div class="relative flex flex-1 flex-col justify-between">
                            <div class="sm:col-gap-5 sm:grid sm:grid-cols-2">
                              <div class="pr-8 sm:pr-5">
                                <p class="text-base font-semibold text-gray-900"> {scrapInfo?.scrapInfo.scrapName}</p>
                                <p class="mx-0 mt-1 mb-0 text-sm text-gray-400">{order?.addressInfo.address}</p>
                              </div>

                              <div class="mt-4 flex items-end justify-between sm:mt-0 sm:items-start sm:justify-end">
                                <p class="shrink-0 w-20 text-base font-semibold text-gray-900 sm:order-2 sm:ml-8 sm:text-right">₹ {order?.finalAmount}</p>
                  ...
                  <div class="mt-6 flex text-center justify-end  space-x-4 ">
                    ...
                    <button onClick={() => handleTrackOrder(order?.orderId)} className="lg:w-[200px] rounded-[30px] h-[50px] font-semibold text-white bg-[#81D742] cursor-pointer max-sm:w-[100px] max-er:text-[10px] lg:text-[15px] max-md:w-[120px] max-er:w-[130px] p-3">
                      Track Order
                    </button>
                  </div>
                </div>
